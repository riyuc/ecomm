...
func withTestDB(t *testing.T, fn func(*sqlx.DB, sqlmock.Sqlmock)) {
	...
	if err != nil {
		t.Fatalf("error creating stub database connection: %v", err)
	}
	defer mockDb.Close()
	...
	tcs := []struct {
	...
	} {
		{
			...
			test: func(t *testing.T, st *MySQLStorer, mock sqlmock.Sqlmock) {
				...
				product, err := st.CreateProduct(context.Background(), p)
...
func TestDeleteProduct(t *testing.T) {
	tcs := []struct {
		...
		},
		{
			...
			test: func(t *testing.T, st *MySQLStorer, mock sqlmock.Sqlmock) {
				mock.ExpectExec("DELETE FROM products WHERE id=?").WillReturnError(fmt.Errorf("error deleting product"))
				err := st.DeleteProduct(context.Background(), 1)
				require.Error(t, err)
				err = mock.ExpectationsWereMet()
				require.NoError(t, err)
